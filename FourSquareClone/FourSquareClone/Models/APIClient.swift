...
final class APIClient {
    static func getVenuesByCategory(categoryID: String, lat: Double, lon: Double, completionHandler: @escaping ((Venue?, AppError?) -> Void)) {
        ...
        URLSession.shared.dataTask(with: urlRequest) {(data, response, error) in
            ...
            if let data = data {
                do {
                    let imageData = try JSONDecoder().decode(Venue.self, from: data)
